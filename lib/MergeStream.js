...
MergeStream.prototype.addConnectionsStream = function (connectionsStream) {
  ...
  this._connectionsStreams[index].on('data', function (connection) {
    // Don't process context or departed connection
    if (!connection['@context'] && connection['departureTime'] >= self._departureTime) {
...
// First parameter: name of the stream you want to stop
// Second parameter: callback that ends the stream
MergeStream.prototype.stopConnectionsStream = function (connectionsStreamName, endConnectionStream) {
  this._amountStreams--;
...
MergeStream.prototype._next = function() {
  ...
  while (i <= this._queues.length) {
    if (this._queues[i] && this._queues[i].length > 0) {
      if (this._queues[i][0]['departureTime'].getTime() == this._departureTime.getTime()) {
...
MergeStream.prototype.close = function () {
  ...
  for (var k=0; k<this._connectionsStreams.length; k++) {
    if (this._connectionsStreams[k]) {
      this._connectionsStreams[k] = null;
    }
  }
