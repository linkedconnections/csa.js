...
var ResultStream = function (query, transferTimesFetcher) { //TODO: , footpathsTo) {
    ...
    // https://stackoverflow.com/questions/26445531/unifying-events-and-promises-is-js-node
    ...
    return new Promise((resolve, reject) => {
        ...
        for(let i=0; i < session.nodes.length; i++) {
            for(let member of await session.nodes[i].getMembers()) {
                ...
                that._earliestArrivalTimes[idNear] = {
                    "@id": null, // No connection ID available for walking
...
ResultStream.prototype._transform = function (connection, encoding, done) {
    ...
    this._transferTimesFetcher.get(previousConnection, connection).then(function (transferTime) {
        ...
        if (self._earliestArrivalTimes[departureStop] &&
            ...
            console.debug(new Date(connection["arrivalTime"]).getTime() < new Date(self._earliestArrivalTimes[connection["arrivalStop"]]["arrivalTime"]).getTime());
                ...
                getNearestStops(stopLocation.latitude, stopLocation.longitude).then(async function (q) {
                    q.on('member', async function (member) {
                        ...
                        that._earliestArrivalTimes[idNear] = {
                            ...
                            "arrivalTime": Math.min(that._earliestArrivalTimes[idNear], addSeconds(connection["arrivalTime"], getWalkingTime(distance))),
