        ...
        session.then(async function (session) {
            ...
            for (let i = 0; i < session.nodes.length; i++) {
                for (let member of await session.nodes[i].getMembers()) {
                    ...
                    let idNear = getStopIdFromPosition(latNear, longNear);
                    if (idNear != self._departureStop) { // The departure stop itself is already included above in the this._earliestArrivalTime
                        self._earliestArrivalTimes[idNear] = {
                            "@id": "footpath:" + idNear, // No connection ID available for walking
                        ...
                        self._minimumSpanningTree[self._earliestArrivalTimes[idNear]["@id"]] = {
    ...
    this._transferTimesFetcher.get(previousConnection, connection).then(function (transferTime) {
        ...
        if (self._earliestArrivalTimes[departureStop] &&
            ...
            if (new Date(connection["arrivalTime"]).getTime() < new Date(self._earliestArrivalTimes[connection["arrivalStop"]]["arrivalTime"]).getTime()) {
                ...
                let stopData = getStopDataFromId(connection["arrivalStop"]);
                let stopLocation = { "latitude": stopData.latitude, "longitude": stopData.longitude };
                getNearestStops(stopLocation.latitude, stopLocation.longitude).then(async function (q) {
                    q.on('member', async function (member) {
                        ...
                        let idNear = getStopIdFromPosition(latNear, longNear);
                        self._earliestArrivalTimes[idNear] = {
                            "@id": idNear, // Needs a connection ID instead of a stop ID
                            "arrivalTime": Math.min(self._earliestArrivalTimes[idNear], addSeconds(connection["arrivalTime"], getWalkingTime(distance))),
                            "gtfs:trip": "walking" // To spot easily the walking fragments during debugging
                        };

                        // Insert into the minimumspanning tree
                        connection.previous = self._earliestArrivalTimes[departureStop]["@id"];
                        self._minimumSpanningTree[connection["@id"]] = connection;
                        //console.log(JSON.stringify(self._earliestArrivalTimes, null, 4))
                    });
