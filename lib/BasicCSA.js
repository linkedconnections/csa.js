...
function getStopData(id) {
    ...
    stopsWestVlaanderen["@graph"].forEach(stop => {
        if(stop["@id"] == id) {
            data = stop;
        }
    });
...
var ResultStream = function (query, transferTimesFetcher) { //TODO: , footpathsTo) {
    ...
    let departureStopData = getStopData(this._departureStop);
    ...
    getNearestStops(departureStopLocation.latitude, departureStopLocation.longitude).then(async function(q) {
        q.on('member', async function(member){
            ...
            ldobj = ldobj[0]
            let str = ldobj["http://xmlns.com/foaf/0.1/name"][0]["@value"]
            let longNear = ldobj["http://www.w3.org/2003/01/geo/wgs84_pos#long"][0]["@value"]
            let latNear = ldobj["http://www.w3.org/2003/01/geo/wgs84_pos#lat"][0]["@value"]
            let distance = getDistance(departureStopLocation.latitude, departureStopLocation.longitude, latNear, longNear);
            nearestStops.push([latNear, longNear, distance]);
